...
class MasterITDashboard:
    ...
    def clean_update_and_push(self):
        ...
        app_path = r"C:\Reminder_v2\Cash_Pick_Up_Reminder_v2.exe"
        ...
        def deploy_worker(store):
            ...
            # 0. Kill lingering hung instances and rename old executable if present
            run_ssh_command(ip, user, pwd, 'taskkill /F /IM Cash_Pick_Up_Reminder_v2.exe')
            rename_cmd = 'powershell -Command "if (Test-Path \'C:\\Reminder_v2\\Cash pick reminder v2.exe\') { Rename-Item -Path \'C:\\Reminder_v2\\Cash pick reminder v2.exe\' -NewName \'Cash_Pick_Up_Reminder_v2.exe\' }"'
            run_ssh_command(ip, user, pwd, rename_cmd)

            # 1. Detect the interactive desktop user via explorer.exe (reliable) instead of parsing 'query user'
            ...
            if not raw_user:
                log("WARNING: no interactive desktop user detected (nobody logged in?)")
