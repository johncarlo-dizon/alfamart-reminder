...
EXE_PATH = r"C:\Reminder_v2\Cash_Pick_Up_Reminder_v2.exe"
...
class InstallationWindow:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Reminder v2 - Setup & Time Synchronizer")
        ...
        # Increased height from 360 to 420 to accommodate the OK button across all DPI scalings
        window_width = 540
        window_height = 420
        ...
        # Logs Console Frame (height set to 10 lines to leave space for OK button)
        ...
        # Action Button Frame (Explicitly packed at the bottom)
    ...
    def run_setup(self):
        ...
        # Step 2: Register Default Shift Tasks
    # Step 2: Register Default Shift Tasks via PowerShell
        ...
        for idx, shift in enumerate(DEFAULT_SHIFTS):
            ...
            # Clean PowerShell script block
            # Registered under $env:USERNAME or 'Users' so the UI displays on screen
            ps_script = f'''
            ...
            # CREATE_NO_WINDOW suppresses the black PowerShell console popup
            res = subprocess.run(
                ps_cmd,
        ...
        # Save Flag
        try:
            with open(r"C:\Windows\Temp\reminder_v2_initialized.flag", "w") as f:
                f.write("initialized")
        except Exception:
            pass
...
def show_reminder_gui(title, message):
    """Displays the custom White Theme Reminder Dialog."""
    root = tk.Tk()
    root.title(title)
    ...
    header_label = tk.Label(
        header_frame, text=title, font=("Segoe UI", 12, "bold"),
        fg="#000000", bg="#FFFFFF"
    )
    header_label.pack()

    body_frame = tk.Frame(root, bg="#FFFFFF", padx=25, pady=10)
    ...
    btn_frame = tk.Frame(root, bg="#FFFFFF", pady=15)
...
def main():
    parser = argparse.ArgumentParser(description="Cash Pick Reminder Popup")
    ...
    flag_path = r"C:\Windows\Temp\reminder_v2_initialized.flag"
    ...
    # Window Titles and Messages when triggered by scheduler
    title = "Cash Pick Reminder"
    ...
    show_reminder_gui(title, message)
